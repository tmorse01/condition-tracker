...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn("id", "uuid", (col) => col.primaryKey())
    .addColumn("loanId", "uuid", (col) =>
      col.notNull().references("Loan.id").onDelete("cascade"),
    )
  ...
  await db.schema
    ...
    .addColumn("id", "uuid", (col) => col.primaryKey())
    .addColumn("conditionId", "uuid", (col) =>
      col.notNull().references("Condition.id").onDelete("cascade"),
    )
    .addColumn("documentId", "uuid", (col) =>
      col.notNull().references("Document.id").onDelete("cascade"),
    )
    .addColumn("documentVersionId", "uuid", (col) =>
      col.notNull().references("DocumentVersion.id").onDelete("cascade"),
    )
    .addColumn("status", "varchar(32)", (col) => col.notNull())
    .addColumn("createdAt", "timestamptz", (col) =>
      col.notNull().defaultTo(sql`now()`),
